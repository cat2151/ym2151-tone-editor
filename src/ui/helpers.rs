...
///
/// Attack and Decay-1 window widths are **proportional to the inverse of their rate**,
/// so a slow attack (low AR) visually occupies a wider segment than a fast one —
/// making the speed of each phase readable from the slope of the polyline.
///
/// Parameters used from `row`:
/// - `PARAM_AR`  (0–31): attack rate – higher = faster rise; 0 = no attack (silent).
/// - `PARAM_D1R` (0–31): decay-1 rate – higher = faster fall toward `D1L`; 0 = no decay.
...
pub(crate) fn compute_op_envelope_points(row: &[u8; GRID_WIDTH]) -> Vec<(f64, f64)> {
    ...
    // TL=0 → 1.0 (full output); TL=99 → 0.0 (near silent).
    let amplitude = (1.0 - tl / tl_max).clamp(0.0, 1.0);

    // Attack phase:
    //   AR=0  → no attack; the operator is completely silent.
    //   AR>0  → always reaches full amplitude.  The window width scales with
    //           (1 − AR/AR_max): fast AR (=31) → 0.05 (near-instant);
    //           slow AR (=1)  → ≈0.24 (gradual rise visible as a shallow slope).
    let attack_peak = if ar > 0.0 { amplitude } else { 0.0 };
    let t_attack_end = if ar > 0.0 {
        0.05 + 0.20 * (1.0 - ar / ar_max)
    } else {
        0.0
    };

    // Decay-1 phase:
    //   D1R=0 → no decay; level stays at attack_peak until note-off (zero-width segment).
    //   D1R>0 → window width scales with (1 − D1R/D1R_max): fast D1R (=31) → 0.05;
    //           slow D1R (=1) → ≈0.24 (gradual fall visible as a shallow slope).
    let t_decay1_span = if d1r > 0.0 {
        0.05 + 0.20 * (1.0 - d1r / d1r_max)
    } else {
        0.0
    };
    ...
    // Gating on attack_peak prevents misleading levels when AR=0.
    let sustain_target = if attack_peak > 0.0 {
        amplitude * (1.0 - d1l / d1l_max).max(0.0)
    } else {
        0.0
    };

    // Level at end of Decay-1.
    //   D1R=0 or no attack → stays at attack_peak (0.0 when AR=0).
    //   D1R>0 → interpolates from attack_peak toward sustain_target.
    let level_end_d1 = if d1r > 0.0 && attack_peak > 0.0 {
        (attack_peak - (attack_peak - sustain_target) * (d1r / d1r_max)).clamp(
    ...
    // D2R=0 → no change; D2R=d2r_max → drops by up to ~50% of current level.
    let level_at_noteoff = (level_end_d1 * (1.0 - d2r / d2r_max * 0.5)).max(0.0);

    // Release midpoint to show RR effect: high RR → mostly gone by midpoint.
