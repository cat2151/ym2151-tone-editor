...
pub fn play_tone(values: &ToneData, use_interactive_mode: bool, cursor_x: usize, cursor_y: usize) {
    if use_interactive_mode {
        log_verbose(&format!("play_tone: Interactive mode - cursor_x={}, cursor_y={}", cursor_x, cursor_y));
...
fn send_all_registers(values: &ToneData) {
    ...
    log_verbose(&format!("send_all_registers: Sending {} register writes", events.len()));
...
fn send_interactive_update(values: &ToneData, _cursor_x: usize, cursor_y: usize) {
    ...
    if cursor_y == ROW_CH {
        ...
        log_verbose(&format!("send_interactive_update: Sending operator registers for data_row={}", data_row));
...
fn send_operator_registers(values: &ToneData, data_row: usize) {
    ...
    log_verbose(&format!("  write_register: addr=0x{:02X}, data=0x{:02X} (DT={}, MUL={})", 0x40 + op_offset as u8, dt_mul, dt, mul));
    let _ = ym2151_log_play_server::client::write_register(0.0, 0x40 + op_offset as u8, dt_mul);
    
    // TL - Register $60-$7F
    let tl = values[data_row][PARAM_TL];
    log_verbose(&format!("  write_register: addr=0x{:02X}, data=0x{:02X} (TL={})", 0x60 + op_offset as u8, tl & 0x7F, tl));
    ...
    log_verbose(&format!("  write_register: addr=0x{:02X}, data=0x{:02X} (KS={}, AR={})", 0x80 + op_offset as u8, ks_ar, ks, ar));
    ...
    log_verbose(&format!("  write_register: addr=0x{:02X}, data=0x{:02X} (AMS={}, D1R={})", 0xA0 + op_offset as u8, ams_d1r, ams, d1r));
    ...
    log_verbose(&format!("  write_register: addr=0x{:02X}, data=0x{:02X} (DT2={}, D2R={})", 0xC0 + op_offset as u8, dt2_d2r, dt2, d2r));
    ...
    log_verbose(&format!("  write_register: addr=0x{:02X}, data=0x{:02X} (D1L={}, RR={})", 0xE0 + op_offset as u8, d1l_rr, d1l, rr));
...
fn send_channel_registers(values: &ToneData) {
    ...
    log_verbose(&format!("  write_register: addr=0x{:02X}, data=0x{:02X} (ALG={}, FB={})", 0x20 + channel as u8, rl_fb_con, alg, fb));
    ...
    // KC - Register $28-$2F
    log_verbose(&format!("  write_register: addr=0x{:02X}, data=0x{:02X} (KC from MIDI note={})", 0x28 + channel as u8, kc, midi_note));
    let _ = ym2151_log_play_server::client::write_register(0.0, 0x28 + channel as u8, kc);
    
    // KF - Register $30-$37
    log_verbose(&format!("  write_register: addr=0x{:02X}, data=0x{:02X} (KF from MIDI note={})", 0x30 + channel as u8, kf, midi_note));
    ...
    log_verbose(&format!("  write_register: addr=0x08, data=0x{:02X} (KEY_ON, slot_mask=0x{:02X})", key_on, slot_mask));
